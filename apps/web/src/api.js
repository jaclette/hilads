...
// The taker's own rating + note (from the reveal modal), stored on their
// acceptance. Mirror of submitHostRating.
export async function submitTakerRating(challengeId, stars, comment) {
  const res = await fetch(`${BASE}/challenges/${encodeURIComponent(challengeId)}/taker-rating`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    credentials: 'include',
    body: JSON.stringify({ stars, ...(comment && comment.trim() ? { comment: comment.trim() } : {}) }),
