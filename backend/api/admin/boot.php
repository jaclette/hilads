...
} elseif (preg_match('#^/admin/messages/([a-zA-Z0-9]+)/delete$#', $uri, $m) && $method === 'POST') {
    $messageId = $m[1];
...
} elseif (preg_match('#^/admin/reports/(\d+)/ban-guest$#', $uri, $m) && $method === 'POST') {
    $reportId = (int) $m[1];
    require __DIR__ . '/report_ban_guest.php';
...
} elseif ($uri === '/admin/thumbs/backfill' && $method === 'POST') {
    require __DIR__ . '/thumbs_backfill.php';
