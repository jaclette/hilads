...
// Day filter. `date` (YYYY-MM-DD) is explicit when present; the per-city
// activity view falls back to today (UTC). Day boundaries are computed in UTC
// for determinism. helper inline to avoid redeclare across includes.
...
<?php if ($channel !== ''):
    ...
    // Optional day scoping - applied only when a date was explicitly passed
    // (e.g. opened from the per-city activity table). Search → channel keeps
    ...
    // Total real (user) messages - excludes system/join/weather.
    $cntStmt = $pdo->prepare("
    ...
    // Back target: to the day activity view if we came from there, else search.
    $backHref = $dateValid !== ''
        ? '/admin/messages?date=' . urlencode($dateValid)
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
        <h1 class="page-title" style="margin-bottom:0">
            ...
            <span style="color:#555;font-size:14px;font-weight:400">
                <?= number_format($total) ?> messages<?php if ($dateValid !== ''): ?> on <?= htmlspecialchars($dateValid, ENT_QUOTES) ?><?php endif; ?>
            </span>
        </h1>
    ...
    <div class="table-wrapper">
        <table>
            ...
            <?php if (empty($messages)): ?>
                <tr><td colspan="4" class="no-results">No messages<?php if ($dateValid !== ''): ?> on this day<?php endif; ?>.</td></tr>
                    ...
                    <td>
                        <?php if (!$isDeleted): ?>
                            <form method="POST" action="/admin/messages/<?= urlencode($msg['id']) ?>/delete"
                                ...
                                <input type="hidden" name="date" value="<?= htmlspecialchars($dateValid, ENT_QUOTES) ?>">
    ...
    <?php if ($pages > 1): ?>
        <div class="pagination">
            <?php $qs = 'channel=' . urlencode($channel) . '&q=' . urlencode($search) . '&type=' . urlencode($typeF) . '&date=' . urlencode($dateValid); ?>
            <?php if ($page > 1): ?>
...
<?php else:
    // ── Default: per-city message counts for a chosen day ───────────────────
    [$ds, $de] = $dayBounds($activityDate);
    // INNER JOIN → only cities that actually have user messages that day.
    ...
    $totalMsgs = 0;
    foreach ($cities as $row) { $totalMsgs += (int) $row['cnt']; }

    // Prev / today / next day nav.
    $prevDate = (new DateTime($activityDate))->modify('-1 day')->format('Y-m-d');
    $nextDate = (new DateTime($activityDate))->modify('+1 day')->format('Y-m-d');
    $today    = gmdate('Y-m-d');
    ?>
    <h1 class="page-title">Messages <span style="color:#555;font-size:14px;font-weight:400">moderation</span></h1>
    <p style="color:#777;margin:-8px 0 16px;font-size:13px">Search a channel by name, or browse a day's city activity below — click a city to open and moderate its messages.</p>

    <?= flash_html() ?>

    <?php include __DIR__ . '/_messages_search_form.php'; ?>

    <!-- City activity by day -->
    <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:14px">
        <h2 style="margin:0;font-size:16px;font-weight:700;color:#eee">🏙️ City activity</h2>
        <form method="GET" action="/admin/messages" style="display:flex;align-items:center;gap:8px;margin:0">
            ...
            <?php if ($activityDate !== $today): ?>
                <a href="/admin/messages" class="btn btn-secondary btn-sm">Today</a>
            <?php endif; ?>
        </form>
        <span style="color:#666;font-size:13px">
            <?= count($cities) ?> cities · <?= number_format($totalMsgs) ?> messages on <?= htmlspecialchars($activityDate, ENT_QUOTES) ?> (UTC)
        </span>
    </div>
    ...
    <div class="table-wrapper">
        <table>
            ...
            <tbody>
            <?php if (empty($cities)): ?>
                <tr><td colspan="4" class="no-results">No city had messages on <?= htmlspecialchars($activityDate, ENT_QUOTES) ?>.</td></tr>
            <?php else: foreach ($cities as $c):
                ...
                <tr>
                    ...
                    <td>
                        <a href="/admin/messages?channel=<?= urlencode($c['id']) ?>&date=<?= urlencode($activityDate) ?>" class="btn btn-primary btn-sm">View messages →</a>
                    </td>
                </tr>
            <?php endforeach; endif; ?>
            </tbody>
